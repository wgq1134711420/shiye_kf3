...
class Kafka_consumer_s():
    def __init__(self):
        ...
        """
        self.consumer = KafkaConsumer(bootstrap_servers = ["192.168.1.172:9092"],auto_offset_reset='earliest',group_id='Ag_MY_GROUP')
        ...
        self.tp = TopicPartition('a_topic_a', 0)
        self.page = 0
    ...
    def mysql_l(self):
        ...
        if self.companyName:
            ...
            return ""
    def kafka_log_offsets(self):
        try:
            with open("/shiye_kf3/gonggao/kafka_stream/logs/log_a_kafka_offsets.log", "w") as w:
                w.write(str(self.end_offset))
        except:
            print("失败")
    def kafka_take_out(self):
        ...
        for each in self.consumer:
            ...
            #     print(each.value, "错误的数据")
                self.end_offset = self.consumer.end_offsets([self.tp])[self.tp]
                self.kafka_log_offsets()
    def kafka_data_processing(self,kafka_json):
        ...
        if kafka_json.get("o"):
            ...
            if kafka_set.get("$set"):
                ...
                self.cmpCode = kafka_of.get('stock_code')
                pubTime = kafka_of.get("etl_time")
                srcUrl = kafka_of.get("url")
                srcUrl = srcUrl.replace("www","static")
                self.title = title
                self.srcUrl = srcUrl
