...
class BertClient(object):
    def __init__(self, ip='localhost', port=5555, port_out=5556,
                 ...
                 timeout=-1, mode='NER',max_seq_length=300, ner_model_dir=None):
        ...
        :param mode: model for bert or ner or etc...
        ...
        if type(mode) != str:
            raise ArithmeticError('model params should be str type, but input model type is :%s' %type(mode))
        else:
            self.mode = mode
        ...
        if check_version or show_server_config or check_length:
            ...
            # check model
            if self.mode != s_status['model']:
                raise ArithmeticError('server model is:{}, but client model is :{}'.format(s_status['model'], self.mode))
