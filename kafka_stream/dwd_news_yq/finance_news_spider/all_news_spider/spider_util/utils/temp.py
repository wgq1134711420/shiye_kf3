...
# current_file_path = 'C:\\logger'
log_path = 'D:\\PythonLog'

if not os.path.isdir(log_path):
    os.makedirs(log_path)
logname = os.path.join(log_path, sys.argv[0].split('/')[-1].split('.')[0] + '.log')
# 本地日志文件
fp = SpiderRotatingFileHandler(logname, maxBytes=10 * 1024 * 1024, mode='a', backupCount=100)
...
# json格式文件 为kafka定制
sfp = SpiderRotatingFileHandler(os.path.join(log_path, "bak.log"), maxBytes=2 * 1024 * 1024, backupCount=2,
                                is_json=True)
# 不启用
