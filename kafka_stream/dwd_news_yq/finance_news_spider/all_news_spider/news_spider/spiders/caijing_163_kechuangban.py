...
class MySpider(RedisSpider):
    name = 'caijing_163_kechuangban'
    allowed_domains = ['money.163.com']
    ori_path = settings.get('ORI_PATH')
    encoding = "gbk"
    start_urls = [
        "http://money.163.com/special/00259CPE/data_kechuangban_kechuangban.js?callback=data_callback",
    ]
    headers = {
    ...
    def parse(self, response):
        ...
        for org in org_list:
            if org:
                title = org.get('title')
                ctime = org.get('time')
                org_url = org.get('docurl')
                if title:
                    url = urljoin(start_url,org_url)
                    print(url)
                    ctime = time.strptime(ctime,"%m/%d/%Y %H:%M:%S")
                    ctime = int(time.mktime(ctime))
    ...
    def detail_parse(self, response):
        ...
        gtime = int(time.time())
        main_business = ''
        source = data.xpath('''//a[@class="author"]/div/text()|//a[@id='ne_article_source']/text()''').text().strip()
        webname = '网易财经'
