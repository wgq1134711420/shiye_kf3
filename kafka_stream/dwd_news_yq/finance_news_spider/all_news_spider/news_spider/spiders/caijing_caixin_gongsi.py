...
class MySpider(RedisSpider):
    name = 'caijing_caixin_guancha'
    allowed_domains = ['companies.caixin.com']
    ...
    def parse(self, response):
        ...
        try:
            ...
            return
        org_list = data.xpathall('''//div[@id="listArticle"]/div''')
        # for org in org_list[:5]:
        for org in org_list:
            if org:
                title = org.xpath('''//h4/a/text()''').text().strip()
                ctime = org.xpath('''//span/text()''').replace(u'月','-').replace(u'日','').regex('(\d+-\d+ \d+:\d+)').text().strip()
