...
class MySpider(RedisSpider):
    name = 'caijing_sohu_hongguan'
    allowed_domains = ['business.sohu.com']
    ori_path = settings.get('ORI_PATH')
    encoding = "utf-8"
    start_urls = [
        "http://v2.sohu.com/integration-api/mix/region/6732?size=25&adapter=pc&secureScore=50&page=1&pvId=1570687589210lHEmrxO",
    ]
    headers = {
    ...
    def detail_parse(self, response):
        ...
        url = response.url
        contents = []  # 全部的文本内容
        content_list = data.xpathall('''//article[@id='mp-editor']''')
