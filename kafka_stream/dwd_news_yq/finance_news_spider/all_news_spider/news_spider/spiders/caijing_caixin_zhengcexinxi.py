...
class MySpider(RedisSpider):
    name = 'caijing_caixin_zhengcexinxi'
    allowed_domains = ['economy.caixin.com']
    ori_path = settings.get('ORI_PATH')
    encoding = "utf-8"
    start_urls = [
        "http://economy.caixin.com/policy/",
    ]
    headers = {
    ...
    def parse(self, response):
        ...
        try:
            ...
            return
        org_list = data.xpathall('''//div[@class="stitXtuwen_list"]/dl''')
        # for org in org_list[:5]:
        for org in org_list:
            if org:
                title = org.xpath('''//h4/a/text()''').text().strip()
                ctime = org.xpath('''//dd/span/text()''').text().strip()
                org_url = org.xpath('''//h4/a/@href''').text().strip()
                if title:
    ...
    def detail_parse(self, response):
        ...
        url = response.url
        contents = []  # 全部的文本内容
        content_list = data.xpathall('''//div[@id="Main_Content_Val"]''')
        for con in content_list:
            con = con.text().strip()
            if con:
                contents.append(con)
        content_x = data.xpath('''//div[@id="Main_Content_Val"]''').data
        content_xml = content_x
        label = {}
        img_list = data.xpathall('''//div[@id="Main_Content_Val"]//img''')
        if img_list:
        ...
        table_list = data.xpathall('''//div[@id="Main_Content_Val"]//table''')
        if table_list:
        ...
        gtime = int(time.time())
        main_business = ''
        source = data.xpath('''//div[@id="artInfo"]/a[1]/text()''').text().strip()
        webname = '财新网'
        domain = self.allowed_domains[0]
        uid = add_uuid(url)
        item["collection_name"] = "news_finance_caixin_raw"    # 集合名
        item["url"] = url    # 链接
        item["uid"] = uid    # 去重id
