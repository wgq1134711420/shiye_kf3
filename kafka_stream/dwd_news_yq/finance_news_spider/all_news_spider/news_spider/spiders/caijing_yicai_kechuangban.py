...
class MySpider(RedisSpider):
    name = 'caijing_yicai_kechuangban'
    allowed_domains = ['www.yicai.com']
    ori_path = settings.get('ORI_PATH')
    encoding = "utf-8"
    start_urls = [
        "https://www.yicai.com/news/kechuangban/",
    ]
    headers = {
    ...
    def parse(self, response):
        ...
        for org in org_list:
            if org:
                ...
                ctime = org.xpath('''//div[@class="author"]/span''').regex('(\d+-\d+-\d+ \d+:\d+)').text().strip()
    ...
    def detail_parse(self, response):
        ...
        gtime = int(time.time())
        main_business = ''
        source = data.xpath('''//div[@class="title f-pr"]/p/span/text()''').text().strip()
        webname = '第一财经'
