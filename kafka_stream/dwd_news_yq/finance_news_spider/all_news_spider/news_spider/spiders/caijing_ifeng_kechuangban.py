...
class MySpider(RedisSpider):
    name = 'caijing_ifeng_kechuangban'
    allowed_domains = ['finance.ifeng.com']
    ori_path = settings.get('ORI_PATH')
    encoding = "utf-8"
    start_urls = [
        "https://shankapi.ifeng.com/spring/finance/index/infoData/1-95385-155385-/getInfoDataList?callback=getInfoDataList",
    ]
    headers = {
    ...
    def detail_parse(self, response):
        ...
        url = response.url
        contents = []  # 全部的文本内容
        content_list = data.xpathall('''//div[@class="text-3zQ3cZD4"]''')
        for con in content_list:
            con = con.text().strip()
            if con:
                contents.append(con)
        content_x = data.xpath('''//div[@class="text-3zQ3cZD4"]''').data
        content_xml = content_x
        label = {}
        img_list = data.xpathall('''//div[@class="text-3zQ3cZD4"]//img''')
        if img_list:
        ...
        table_list = data.xpathall('''//div[@class="text-3zQ3cZD4"]//table''')
        if table_list:
        ...
        gtime = int(time.time())
        main_business = ''
        source = data.xpath('''//span[@class="source-2pXi2vGI"]/a/text()''').text().strip()
        webname = '凤凰财经'
        domain = self.allowed_domains[0]
        uid = add_uuid(url)
        item["collection_name"] = "news_finance_ifeng_raw"    # 集合名
        item["url"] = url    # 链接
        item["uid"] = uid    # 去重id
