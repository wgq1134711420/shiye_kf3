...
class MySpider(RedisSpider):
    name = 'news_sina_china_guonei'
    allowed_domains = ['finance.sina.com.cn']
    ori_path = settings.get('ORI_PATH')
    encoding = "utf-8"
    start_urls = ["http://interface.sina.cn/pc_api/public_news_data.d.json?cids=239198%2C239199%2C239200%2C239201%2C239202&pdps=&smartFlow=&editLevel=0%2C1%2C2%2C3&type=std_news%2Cstd_slide%2Cstd_video&pageSize=20&mod=nt_category_finance_fund_hot&cTime=1483200000&up=0&action=0"]
    headers = {
    ...
    def detail_parse(self, response):
        ...
        gtime = int(time.time())
        main_business = ''
        source = data.xpath('''//span[@class="source ent-source"]/text()''').text().strip()
        webname = '新浪财经'
        domain = self.allowed_domains[0]
        uid = add_uuid(url)
        item["collection_name"] = "news_sina_china_raw"    # 集合名
        item["url"] = url    # 链接
        item["uid"] = uid    # 去重id
