...
data_list = dbname[mongo_name].find({"gtime": {"$gt": 1575216000},"ctime": {"$gt": 1325347200, "$lt": 1575216000}, 'branch_tree.0':'中华人民共和国公安部', 'branch': '鹤壁市公安局'})
...
# data_list = dbname[mongo_name].find({"uid":'47d4bdca79b639c094bc1c9f011b3fe4'})
...
for i, line_de in enumerate(data):
    ...
    '行政拘留' in s or '非法居留' in s or '违法事实可能涉及敏感内容' in s or '醉酒后驾驶机动车' in s, '涉嫌非法入境' in s) and line_de.get('branch_tree')['1'] != '江苏省公安厅' and\
    line_de.get('branch_tree')['1'] != '内蒙古自治区公安厅' and line_de.get('branch_tree')['1'] != '安徽省公安厅' and line_de.get('branch_tree')['1'] != '湖北省公安厅' and line_de.get('branch_tree')['1'] != '广东省公安厅' and line_de.get('branch_tree')['1'] != '河南省公安厅' and line_de.get('branch_tree')['1'] != '四川省公安厅':
        person_company_tmp = []
        company_name_tmp = ['针对个人处罚']
        skip_flag = True
    elif '2' in line_de.get('branch_tree').keys() and line_de.get('branch_tree')['2'] == '烟台市公安局' and ('违法行为人' in s or '违法单位' in s):
        if '违法行为人' in s:
    ...
    if len(line_de.get('accessory')) > 0 and 'file_name' in line_de.get('accessory')[0].keys() and \
        ...
        if '\n' in acce_content_xls and '2' in line_de.get('branch_tree').keys() and line_de.get('branch_tree')['2'] in ['广安市公安局', '江门市公安局']:
    ...
    if line_de.get('branch_tree')['0'] in ['中华人民共和国公安部']:
        ...
        if contents_table_list != []:
            ...
            for row in contents_table_list:
                ...
                len(info_split[1]) < 50 and is_Chinese(info_split[1][:4]) and info_split[1][-1] != '号' and info_split[0].isdigit():
                    skip_flag_list[i] = True
                    tmp_penalty = '0.0'
                    company = info_split[1]
...
for i,item in enumerate(data):
    ...
    # 调用提取金额模块 ###########################可更改###########################
    extract_term = {}
    if skip_flag_list[i] == False or ('2' in line_de.get('branch_tree').keys() and line_de.get('branch_tree')['2'] in ['烟台市公安局', '鹤壁市公安局', '洛阳市公安局'] and company_name_list[i][0] != '针对个人处罚'):
