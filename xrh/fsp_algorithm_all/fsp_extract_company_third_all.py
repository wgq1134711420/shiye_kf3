...
# mongo上的爬取数据
mongo_name = 'fsp_department_merge_third_raw'
...
data_list = dbname[mongo_name_xyzg].find({"gtime": {"$gt": add_type[add_type_choice]},"ctime": {"$gt": 1325347200}, 'branch_tree.0':'信用中国','branch_tree.1':'常州', 'article_info.类型':'失信黑名单'}).limit(10)
...
data_list = dbname[mongo_name_xyzg].find({"uid":'07a413995bb735809d461e8494b07cc1'})
...
for i, line_de in enumerate(data):
        ...
        for num in range(len(line_de.get('accessory'))) :
            ...
            if 'file_content' in line_de.get('accessory')[num].keys():
                ...
                s_p += DBC2SBC(line_de.get('accessory')[num]['file_content']).replace('–','')\
                    ...
                    .replace('—','').replace('.','').replace('_','').replace('/','')\
                    .replace('\n','').replace('\r','') + '\n'
                if len(contents) > 0 and ('xls' in ''.join(contents) or 'doc' in ''.join(contents) or '名单附后' in ''.join(contents)):
                    s_p = '\t'.join(s_p.split())
    ...
    (line_de.get('contents')[0] == 'table_list' or (len(line_de.get('accessory')) > 0 and 'file_name' in line_de.get('accessory')[0].keys())):
        person_company_tmp = []
        company_name_tmp = []
        skip_flag = True

    if '0' in line_de.get('branch_tree').keys() and line_de.get('branch_tree')['0'] == '证券期货市场' and len(t) > 4:
        person_company_tmp = []
        company_name_tmp = [t]
        skip_flag = True
    # else:
            ...
            try:
                ...
                if result_list != []:
                    for co in result_list:
                        if len(co) > 4 and '路' != co[-1:] and co not in company_name_tmp and co.replace('。','') not in company_name_tmp:
    ...
    # 信用中国 失信黑名单==================================================================
    if line_de.get('branch_tree')['0'] in ['信用中国'] and line_de.get('article_info')['类型'] == '失信黑名单':
        # 针对数据特点进行处理
        acce_content_doc = ''
        # 提取附件表格中的信息
        if len(line_de.get('accessory')) > 0 and 'file_name' in line_de.get('accessory')[0].keys() and \
            ...
            for num in range(len(line_de.get('accessory'))):
                ...
                if 'file_content' in line_de.get('accessory')[num] and 'name' in line_de.get('accessory')[num] and \
                    ...
                    acce_content_doc += DBC2SBC(
                        line_de.get('accessory')[num]['file_content']) + '**different_acce**'
            # print('acce_content_doc',[acce_content_doc[:1000]])

            if '\n\n\t1\n' in acce_content_doc:
                s_row_split = split_it(acce_content_doc, split_key_3)
            ...
            for s_row in s_row_split:
                ...
                if '\n\t' in s_row and s_row.count('\n\t') > 3:
                    info_split = s_row.replace(' ', '').split('\n\t')
                    while '' in info_split:
                        info_split.remove('')
                    while '\n' in info_split:
                        info_split.remove('\n')
        ...
        if contents_table_list != []:
            ...
            for row in contents_table_list:
                ...
                if line_de.get('branch') in ['信用中国(鸡西)'] and '酒驾' not in t and '个人' not in t and '无证行医' not in t and\
                ...
                elif line_de.get('branch') in ['信用中国(鸡西)'] and '酒驾' not in t and '个人' not in t and '无证行医' not in t and\
                len(info_split) == 9 and len(info_split[-1]) > 4 and\
                len(info_split[-1]) < 50 and is_Chinese(info_split[-1][:4]) and info_split[1].isdigit():
                ...
                len(info_split[2]) < 50 and is_Chinese(info_split[2][:4]) and info_split[0].isdigit():
                ...
                len(info_split[1]) < 50 and is_Chinese(info_split[1][:4]):
                    ...
                    company_name_tmp.append(info_split[1])
                    punish_type_tmp.append(info_split[0])
                    penalty_amount_tmp.append('0.0')
                    involved_amount_tmp.append('0.0')
                    punish_detail_tmp.append(row[:1000])
                    event_num_tmp.append('')
                    etime_tmp.append(info_split[9])
                elif line_de.get('branch') in ['信用中国(盐城)'] and\
                ...
                len(info_split[3]) < 50 and is_Chinese(info_split[3][:4]) and info_split[0].isdigit():
                    ...
                    punish_detail_tmp.append(row[:1000])
                    event_num_tmp.append(info_split[2])
                    etime_tmp.append(info_split[12])
                elif line_de.get('branch') in ['信用中国(盐城)'] and\
                ...
                len(info_split[2]) < 50 and is_Chinese(info_split[2][:4]) and info_split[0].isdigit():
                    ...
                    end_time_tmp.append(info_split[11])
                elif line_de.get('branch') in ['信用中国(常州)'] and\
                ...
                len(info_split[3]) < 50 and is_Chinese(info_split[3][:4]) and info_split[0].isdigit():
                    ...
                    end_time_tmp.append(info_split[8])
...
for i,item in enumerate(data):
    ...
    if len(item['accessory']) > 0:
        for acc in item['accessory']:
            if 'file_content' in acc.keys() and 'name' in acc.keys() and\
                '许可' not in acc['name'] and '征求意见稿' not in acc['name'] and '认可' not in acc['name']:
    ...
    if eventNum1 == '' and len(item['contents']) > 10 and '行政处罚决定书文号' in item['contents']:
        ...
        if len(item['contents']) > tmp_pos1 and len(item['contents'][tmp_pos1]) < 20 and '号' in item['contents'][tmp_pos1]:
    ...
    ')' in title and '号' in title:
        ...
        if len(title[tmp_pos1+1:tmp_pos2]) < 20 and '号' in title[tmp_pos1+1:tmp_pos2]:eventNum1 = title[tmp_pos1+1:tmp_pos2]
    ...
    item['relate_amount']['punish_amount'] != '' and item['relate_amount']['punish_amount'][0].isdigit():
    ...
    involvedAmount1 = 0
    if 'relate_amount' in item.keys() and 'other_amount' in item['relate_amount'].keys() and item['relate_amount']['other_amount'] != '':
        involvedAmount1 = round(float(han_2_num.cn2digTransformer(item['relate_amount']['other_amount'])),2)
    if involvedAmount1 == 0 and 'relate_amount' in item.keys() and 'owing_amount' in item['relate_amount'].keys() and item['relate_amount']['owing_amount'] != '':
    ...
    if company_name_list[i] != [''] and company_name_list[i] != '' and company_name_list[i] != []:
        ...
        for j,company in enumerate(company_name_list[i]):
            ...
            if skip_flag_list[i]:
                ...
                if len(penalty_amount_list) > i and len(penalty_amount_list[i]) > j and penalty_amount_list[i][j].replace('.','').isdigit() and penalty_amount_list[i][j].count('.') <= 1:
                    penaltyAmount = penalty_amount_list[i][j]
                if len(involved_amount_list) > i and len(involved_amount_list[i]) > j and involved_amount_list[i][j].replace('.','').isdigit() and involved_amount_list[i][j].count('.') <= 1:
