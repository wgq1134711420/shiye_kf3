...
# 总表名
# sy_csc_fsp_department_all 正式表
# sy_csc_fsp_department_daily 增量表
# sy_csc_fsp_department_all_tmp_xrh 实验表
final_mysql_name = 'sy_csc_fsp_department_all_tmp_xrh'
...
dupli_uid_list = []
# if add_type_choice == 'all':
#     # 如果增量处理，取log里的时间戳
#     cursor.execute("SELECT distinct uid FROM " + 'sy_csc_fsp_department_all' + " WHERE branch0 in ('信用中国') and eventType = '失信黑名单' and gtimeDatetime > '2019-12-01'",)
#     l1 = list(cursor.fetchall())
#     cn = [desc[0] for desc in cursor.description]
#     # 初始化max_gtime
#     for i in l1:
#         dupli_uid_list.append(i[cn.index('uid')])
# print('dupli_uid_list长度:', len(dupli_uid_list))
#
# data_list = dbname[mongo_name_xyzg].find({"uid": {"$nin": dupli_uid_list},"gtime":{"$lt":1575936000, "$gt": 1575129600}, "ctime": {"$gt": 1325347200, "$lt":1575129600}})
data_list = dbname[mongo_name_xyzg].find(
    {"gtime": {"$gt": 1582214400}, 'branch_tree.0': '信用中国', 'branch': '信用中国(安康)'})

# data_list = dbname[mongo_name_xyzg].find(
#     {"gtime": {"$gt": 1579017600}, 'branch_tree.0': '信用中国', 'branch': '信用中国(上海)'})
# data_list = dbname[mongo_name_xyzg].find(
...
data_list = dbname[mongo_name_xyzg].find({"uid":'57f8aa0a94433c5da1746261b1e0e0f1'})
...
for i, line_de in enumerate(data):
    ...
    if len(contents) == 2 and line_de.get('branch') == '信用中国(信阳)':
        contents_table_list = []
    if line_de.get('branch') in ['信用中国(汕头)', '信用中国(南宁)', '信用中国(新乡)', '信用中国(咸阳)', '信用中国(酒泉)'] and line_de.get('uid') != 'd50f14fcf65735329f9af9372730a4ce':
        contents_table_list = []
            ...
            if '处罚结果' in contents:
                ...
                if extract_term and t in extract_term.keys():
                    if 'penaltyAmount' in extract_term[t].keys() and \
                        ...
                        tmp_penalty = str(round(float(extract_term[t]['penaltyAmount']), 2))
                        penalty_amount_tmp.append(tmp_penalty)
        if line_de.get('branch') == '信用中国(韶关)':
            if '发布年度：' in contents:
                p4 = contents.index('发布年度：')
                if '20' in contents[p4 + 1] and len(contents[p4 + 1]) ==4:
                    etime = contents[p4 + 1] + '-01-01'
                else:
                    etime = '0001-01-01'
            elif '年度：' in contents:
                p4 = contents.index('年度：')
                if '20' in contents[p4 + 1] and len(contents[p4 + 1]) ==4:
                    etime = contents[p4 + 1] + '-01-01'
                else:
                    etime = '0001-01-01'
            ...
            punish_detail_tmp.append('')
            etime_tmp.append(etime)
        skip_flag = True
    elif line_de.get('branch') in ['信用中国(北京)', '信用中国(亳州)', '信用中国(蚌埠)', '信用中国(淮南)', '信用中国(芜湖)', '信用中国(铜陵)', '信用中国(池州)', '信用中国(莆田)', '信用中国(鹰潭)', '信用中国(抚州)', '信用中国(淄博)', '信用中国(菏泽)', '信用中国(驻马店)', '信用中国(长沙)', '信用中国(岳阳)', '信用中国(张家界)', '信用中国(邵阳)', '信用中国(衡阳)', '信用中国(韶关)', '信用中国(河源)', '信用中国(潮州)', '信用中国(佛山)', '信用中国(东莞)', '信用中国(中山)', '信用中国(百色)', '信用中国(攀枝花)', '信用中国(南昌)', '信用中国(沧州)', '信用中国(苏州)'] and len(t) < 4:
        person_company_tmp = []
    ...
    if line_de.get('branch') == '信用中国(河北)':
        if '企业名称：' in contents:
            ...
            skip_flag = True
        if line_de.get('branch') == '信用中国(中山)' and '企业名称' in contents:
            company_name_tmp = []
            p1 = contents.index('企业名称')
            if '认定日期' in contents:
                p4 = contents.index('认定日期')
                if '20' in contents[p4 + 1] and len(contents[p4 + 1]) >=8 and len(contents[p4 + 1]) <= 10:
                    etime = contents[p4 + 1]
                else:
                    etime = '0001-01-01'
            elif '年度' in contents:
                ...
                etime = '0001-01-01' 
            if '公布期限' in contents:
                p5 = contents.index('公布期限')
                if '20' in contents[p5 + 1] and len(contents[p5 + 1]) >=8:
                    end_time = contents[p5 + 1]
                else:
                    end_time = '0001-01-01'
            else:
                end_time = '0001-01-01'
    ...
    if line_de.get('branch') == '信用中国(萍乡)' and '失信被执行人姓名或名称：' in contents:
        ...
        if '立案时间：' in contents:
            ...
            e_time = '0001-01-01'
        if '发布时间：' in contents:
            p6 = contents.index('发布时间：')
            c_time = contents[p6 + 1]
            if '20' not in c_time or len(c_time) > 10:
                c_time = '0001-01-01'
        else:
            c_time = '0001-01-01'
        if '案号：' in contents:
    ...
    if len(line_de.get('accessory')) > 0 and 'file_name' in line_de.get('accessory')[0].keys() and \
            '.xls' in line_de.get('accessory')[0]['file_name'] and line_de.get('branch') not in ['信用中国(阜阳)','信用中国(潍坊)', '信用中国(湛江)', '信用中国(南阳)']:
        print('xls')
        for num in range(len(line_de.get('accessory'))):
            # print(num)
        ...
        for s_row in s_row_split:
            ...
            if info_split:
                ...
                if line_de.get('branch') == '信用中国(成都)' and \
                        len(info_split) == 9 and len(info_split[2]) > 4 and \
                        len(info_split[2]) < 50 and is_Chinese(info_split[2][:4]) and \
                        '名称' not in info_split[2] and '企业' not in info_split[2] and \
                        '法人' not in info_split[2] and '自然人' not in info_split[2]:
                    skip_flag_list[i] = True
                    punish_type = get_punish_type(info_split[0])
                ...
                if line_de.get('branch') == '信用中国(上海)' and \
                    ...
                    punish_type = get_punish_type(s_row)
                    company_name_tmp.append(company)
            ...
            for row in contents_table_list:
                ...
                len(info_split[6]) < 50 and is_Chinese(info_split[6][:4]) and '企业名称:' in info_split[6]:
                    ...
                    if '立案时间:' in info_split[0]:
                        ...
                        e_time = ''
                    if '执行法院:' in info_split[7]:
                        branch_name = info_split[7].split(':')[-1]
                    else:
                        branch_name = '信用中国'
                    company_name_tmp.append(company)
                    punish_type_tmp.append(punish_type)
                ...
                len(info_split[6]) < 50 and is_Chinese(info_split[6][:4]) and '对象名称:' in info_split[6]:
                    ...
                    if '涉及金额（元）:' in info_split[9]:
                        ...
                        involved_amount = '0.0'
                    if '认定部门:' in info_split[8]:
                        branch_name = info_split[8].split(':')[-1]
                    else:
                        branch_name = '信用中国'
                    company_name_tmp.append(company)
                    punish_type_tmp.append(punish_type)
                ...
                len(info_split) >= 23 and '执行案号：' in info_split and '发布时间：' in info_split:
                    ...
                    if len(info_split[p2 + 1]) == 10:
                        ...
                        e_time = '0001-01-01'
                    p3 = info_split.index('作出执行依据单位：')
                    if len(info_split[p3 + 1]) > 3:
                        branch_name = info_split[p3 + 1]
                    else:
                        branch_name = '信用中国(乌海)'
                    skip_flag_list[i] = True
                ...
                len(info_split[1]) < 50 and is_Chinese(info_split[1][:4]) and '自然人' not in info_split:
                    ...
                    if '20' in info_split[10]:
                        ...
                        end_time = '0001-01-01'
                    if '法院' in info_split[7] or '局' in info_split[7]:
                        branch_name = info_split[7]
                    else:
                        branch_name = '信用中国(鸡西)'
                    punish_type_tmp.append('')
                ...
                len(info_split[1]) < 50 and is_Chinese(info_split[1][:4]):
                    ...
                    if '号' in info_split[6]:
                        ...
                        event_num = ''
                    if '法院' in info_split[11]:
                        branch_name = info_split[11]
                    else:
                        branch_name = '信用中国(南通)'
                    punish_type_tmp.append('')
                    penalty_amount_tmp.append('0.0')
                    ...
                    branch_name_tmp.append(branch_name)
                if line_de.get('branch') in ['信用中国(南通)'] and\
                        (len(info_split) == 14 or len(info_split) == 15) and len(info_split[3]) > 4 and\
                len(info_split[3]) < 50 and is_Chinese(info_split[3][:4]):
                    skip_flag_list[i] = True
                    company = info_split[3]
                    company_name_tmp.append(company)
                    ...
                    if '号' in info_split[2]:
                        ...
                        event_num = ''
                    if '法院' in info_split[1] or '局'in info_split[1]:
                        branch_name = info_split[1]
                    else:
                        branch_name = '信用中国(南通)'
                    punish_type_tmp.append('')
                ...
                if line_de.get('branch') in ['信用中国(南通)'] and\
                len(info_split) == 17 and len(info_split[2]) > 4 and\
                len(info_split[2]) < 50 and is_Chinese(info_split[2][:4]) and '号' not in info_split[2]:
                    skip_flag_list[i] = True
                    company = info_split[2]
                    company_name_tmp.append(company)
                    if '20' in info_split[11] and len(info_split[11]) <= 10:
                    ...
                    if '号' in info_split[1]:
                        ...
                        event_num = ''
                    if '法院' in info_split[14]:
                        branch_name = info_split[14]
                    else:
                        branch_name = '信用中国(南通)'
                    punish_type_tmp.append('')
                    penalty_amount_tmp.append('0.0')
                    ...
                    branch_name_tmp.append(branch_name)
                if line_de.get('branch') in ['信用中国(南通)'] and\
                        (len(info_split) == 16 or len(info_split) == 17) and len(info_split[3]) > 4 and\
                len(info_split[3]) < 50 and is_Chinese(info_split[3][:4]) and '号' in info_split[2]:
                    skip_flag_list[i] = True
                    company = info_split[3]
                    company_name_tmp.append(company)
                    if '20' in info_split[12] and len(info_split[12]) <= 10:
                ...
                if line_de.get('branch') in ['信用中国(南通)'] and\
                len(info_split) == 12 and len(info_split[1]) > 4 and\
                len(info_split[1]) < 50 and is_Chinese(info_split[1][:4]) and '局' not in info_split[1]:
                    skip_flag_list[i] = True
                    company = info_split[1]
                    company_name_tmp.append(company)
                    if '20' in info_split[8] and len(info_split[8]) <= 10:
                    ...
                    if '号' in info_split[5]:
                        ...
                        event_num = ''
                    if '法院' in info_split[4] or '局' in info_split[4]:
                    	branch_name = info_split[4]
                    elif '法院' in info_split[9] or '局' in info_split[9]:
                    	branch_name = info_split[9]
                    else:
                    	branch_name = '信用中国(南通)'
                    punish_type_tmp.append('')
                    ...
                    branch_name_tmp.append(branch_name)
                if line_de.get('branch') in ['信用中国(南通)'] and\
                len(info_split) == 12 and len(info_split[3]) > 4 and\
                len(info_split[3]) < 50 and is_Chinese(info_split[3][:4]) and '号' in info_split[2]:
                    skip_flag_list[i] = True
                    company = info_split[3]
                    company_name_tmp.append(company)
                    ...
                    branch_name_tmp.append(branch_name)
                if line_de.get('branch') in ['信用中国(南通)'] and\
                        (len(info_split) == 11 or len(info_split) == 9) and len(info_split[3]) > 4 and\
                len(info_split[3]) < 50 and is_Chinese(info_split[3][:4]) and '法院' not in info_split[3] and '办法' not in info_split[3]:
                    skip_flag_list[i] = True
                    company = info_split[3]
                    company_name_tmp.append(company)
                    if '20' in info_split[7] and len(info_split[7]) <= 10:
                ...
                len(info_split[10]) < 50 and '失信被执行人姓名或名称:' in info_split[10]:
                    ...
                    if '案号:' in info_split[16]:
                        ...
                        event_num = ''
                    if '执行法院:' in info_split[13]:
                    	branch_name = info_split[13].split('执行法院:')[-1]
                    company_name_tmp.append(company)
                    punish_type_tmp.append('')
                    penalty_amount_tmp.append('0.0')
                    involved_amount_tmp.append('0.0')
                ...
                len(info_split[12]) < 50 and '失信被执行人姓名或名称:' in info_split[12]:
                    ...
                    if '案号:' in info_split[10]:
                        ...
                        event_num = ''
                    if '执行法院:' in info_split[5]:
                    	branch_name = info_split[5].split('执行法院:')[-1]
                    company_name_tmp.append(company)
                    punish_type_tmp.append('')
                    penalty_amount_tmp.append('0.0')
                    involved_amount_tmp.append('0.0')
                    ...
                    branch_name_tmp.append(branch_name)
                if line_de.get('branch') in ['信用中国(江西)'] and\
                len(info_split) == 11 and len(info_split[6]) > 10 and\
                len(info_split[6]) < 50 and '机构名称:' in info_split[6]:
                    skip_flag_list[i] = True
                    company = info_split[6]
                    company = company.split(':')[-1]
                    if '20' in info_split[8] and '处罚日期:' in info_split[8]:
                        e_time = info_split[8]
                        e_time = e_time.split('处罚日期:')[-1]
                    else:
                        e_time = '0001-01-01'
                    event_num = ''
                    if '处罚机关:' in info_split[7]:
                    	branch_name = info_split[7].split('处罚机关:')[-1]
                    if '处罚结果:' in info_split[0]:
	                    punish_type = get_punish_type(info_split[0])
	                    tmp_penalty = '0.0'
	                    extract_term = num_extract.NumberExtract().dosegment_all(info_split[0], [company])
	                    if extract_term and company in extract_term.keys():
	                        if 'penaltyAmount' in extract_term[company].keys() and \
	                                extract_term[company]['penaltyAmount'] != '':
	                            tmp_penalty = str(round(float(extract_term[company]['penaltyAmount']), 2))
                    ...
                    branch_name_tmp.append(branch_name)
                if line_de.get('branch') in ['信用中国(江西)'] and\
                len(info_split) == 12 and len(info_split[2]) > 10 and\
                len(info_split[2]) < 50 and '企业名称:' in info_split[2]:
                    skip_flag_list[i] = True
                    company = info_split[2]
                    company = company.split(':')[-1]
                    e_time = '0001-01-01'
                    event_num = ''
                    if '行政处罚决定单位:' in info_split[7]:
                    	branch_name = info_split[7].split('行政处罚决定单位:')[-1]
                    if '处罚结果:' in info_split[2]:
	                    punish_type = get_punish_type(info_split[2])
	                    tmp_penalty = '0.0'
	                    extract_term = num_extract.NumberExtract().dosegment_all(info_split[2], [company])
	                    if extract_term and company in extract_term.keys():
	                        if 'penaltyAmount' in extract_term[company].keys() and \
	                                extract_term[company]['penaltyAmount'] != '':
	                            tmp_penalty = str(round(float(extract_term[company]['penaltyAmount']), 2))
                    ...
                    branch_name_tmp.append(branch_name)
                if line_de.get('branch') in ['信用中国(江西)'] and\
                len(info_split) == 5 and len(info_split[2]) > 10 and\
                len(info_split[2]) < 50 and '企业名称:' in info_split[2]:
                    skip_flag_list[i] = True
                    company = info_split[2]
                    company = company.split(':')[-1]
                    ...
                    etime_tmp.append(e_time)
                if line_de.get('branch') in ['信用中国(江西)'] and\
                len(info_split) == 9 and len(info_split[3]) > 10 and\
                len(info_split[3]) < 50 and '企业名称:' in info_split[3]:
                    skip_flag_list[i] = True
                    company = info_split[3]
                    company = company.split(':')[-1]
                    e_time = '0001-01-01'
                ...
                len(info_split[0]) < 50 and '处罚开始时间:' in info_split[0]:
                    ...
                    if '号' in info_split[1] and '处罚方案描述:' in info_split[1]:
                        ...
                        event_num = ''
                    if '法院' in info_split[3] and '采集部门名称:' in info_split[3]:
                        branch_name = info_split[3]
                        branch_name = branch_name.split(':')[-1]
                    else:
                        branch_name = '信用中国(三门峡)'
                    company_name_tmp.append(company)
                    punish_type_tmp.append('')
                    penalty_amount_tmp.append('0.0')
                    involved_amount_tmp.append('0.0')
                    ...
                    branch_name_tmp.append(branch_name)
                if line_de.get('branch') in ['信用中国(三门峡)'] and\
                len(info_split) == 11 and '立案时间:' in info_split[4]:
                    skip_flag_list[i] = True
                    if len(t) < 4:
                        company = '针对个人处罚'
                    else:
                    ...
                    if '20' in info_split[4] and '立案时间:' in info_split[4]:
                        ...
                        e_time = '0001-01-01'
                    if '20' in info_split[7] and '发布时间:' in info_split[7]:
                        c_time = info_split[7]
                        c_time = c_time.split(':')[-1]
                        if len(c_time) < 8:
                            c_time = '0001-01-01'
                        if c_time[-2:].isdigit() and int(c_time[-2:]) > 31:
                            c_time = '0001-01-01'
                    else:
                        c_time = '0001-01-01'
                    if '失信案号:' in info_split[2]:
                        ...
                        event_num = ''
                    if '法院' in info_split[5] and '执行法院:' in info_split[5]:
                        branch_name = info_split[5]
                        branch_name = branch_name.split(':')[-1]
                    else:
                        branch_name = '信用中国(三门峡)'
                    ...
                    branch_name_tmp.append(branch_name)
                if line_de.get('branch') in ['信用中国(三门峡)'] and\
                len(info_split) == 11 and '立案时间:' in info_split[2]:
                    skip_flag_list[i] = True
                    if len(t) < 4:
                        company = '针对个人处罚'
                    else:
                        company = t
                    if '20' in info_split[2] and '立案时间:' in info_split[2]:
                        e_time = info_split[2]
                        e_time = e_time.split(':')[-1]
                        if len(e_time) < 8:
                            e_time = '0001-01-01'
                        ...
                        e_time = '0001-01-01'
                    if '20' in info_split[5] and '发布时间:' in info_split[5]:
                        c_time = info_split[5]
                        c_time = c_time.split(':')[-1]
                        if len(c_time) < 8:
                            c_time = '0001-01-01'
                        ...
                        c_time = '0001-01-01'
                    if '失信案号:' in info_split[1]:
                        event_num = info_split[1]
                        event_num = event_num.split(':')[-1]
                    else:
                        event_num = ''
                    if '法院' in info_split[3] and '执行法院:' in info_split[3]:
                        branch_name = info_split[3]
                        branch_name = branch_name.split(':')[-1]
                    else:
                        branch_name = '信用中国(三门峡)'
                ...
                len(info_split) == 6 and len(info_split[1]) > 4 and\
                len(info_split[1]) < 50 and is_Chinese(info_split[1][:4]) and '名称' not in info_split[1]:
                    skip_flag_list[i] = True
                    company = info_split[1]
                    branch_name = info_split[4]
                    company_name_tmp.append(company)
                ...
                len(info_split[1]) < 50 and is_Chinese(info_split[1][:4]) and '公司' in info_split[1]:
                    ...
                    if '号' not in event_num:
                        event_num = ''
                    if '法院' in info_split[6]:
                        branch_name = re.findall(r'(.*法院)?', info_split[6])[0]
                    punish_type = ''
                    company_name_tmp.append(company)
                    punish_type_tmp.append(punish_type)
                    penalty_amount_tmp.append(tmp_penalty)
                ...
                len(info_split[3]) < 50 and '公司名称:' in info_split[3] and '信息报送日期'in info_split[0]:
                    skip_flag_list[i] = True
                    company = info_split[3]
                    company = company.split(':')[-1]
                    punish_type = get_punish_type(info_split[4])
                    ...
                    if '案件号：' in info_split[2]:
                        ...
                        event_num = ''
                    if '决定机关:' in info_split[1]:
                    	branch_name = info_split[1]
                    	branch_name = branch_name.split('决定机关:')[-1]
                    else:
                        branch_name = '信用中国(宁夏)'
                    company_name_tmp.append(company)
                    punish_type_tmp.append(punish_type)
                ...
                if line_de.get('branch') in ['信用中国(宁夏)'] and\
                len(info_split) == 5 and len(info_split[3]) > 10 and\
                len(info_split[3]) < 50 and '公司名称:' in info_split[3] and '信息报送日期'in info_split[2]:
                    skip_flag_list[i] = True
                    company = info_split[3]
                    company = company.split(':')[-1]
                    punish_type = get_punish_type(info_split[0])
                    if '20' in info_split[2] and '信息报送日期:' in info_split[2]:
                        e_time = info_split[2]
                        e_time = e_time.split('日期:')[-1]
                        if len(e_time) > 10:
                            e_time = e_time[:10]
                        ...
                        e_time = '0001-01-01'
                    if '案件号：' in info_split[1]:
                        event_num = re.findall(r'案件号：(.*号)?', info_split[1])[0]
                    else:
                        event_num = ''
                    if '决定机关:' in info_split[4]:
                    	branch_name = info_split[4]
                    	branch_name = branch_name.split('决定机关:')[-1]
                    else:
                        branch_name = '信用中国(宁夏)'
                ...
                len(info_split[1]) < 50 and is_Chinese(info_split[1][:4]):
                    ...
                    if '20' in info_split[7] and len(info_split[7]) <= 10:
                        ...
                        e_time = '0001-01-01'
                    if '局' in info_split[4] or '法院' in info_split[4]:
                    	branch_name = info_split[4]
                    else:
                    	branch_name = '信用中国(南通)'
                    company_name_tmp.append(company)
                    ...
                    branch_name_tmp.append(branch_name)
                if line_de.get('branch') in ['信用中国(南通)'] and\
                len(info_split) == 10 and len(info_split[3]) > 4 and\
                len(info_split[3]) < 50 and is_Chinese(info_split[3][:4]) and info_split[0].isdigit():
                    skip_flag_list[i] = True
                    company = info_split[3]
                    if '号' in info_split[2]:
                        event_num = info_split[2]
                    ...
                    branch_name_tmp.append(branch_name)
                if line_de.get('branch') in ['信用中国(南通)'] and\
                len(info_split) == 8 and len(info_split[1]) > 4 and\
                len(info_split[1]) < 50 and is_Chinese(info_split[1][:4]) and info_split[0].isdigit():
                    skip_flag_list[i] = True
                    company = info_split[1]
                    event_num = ''
                    if '20' in info_split[7] and len(info_split[7]) <= 10:
                    ...
                    branch_name_tmp.append(branch_name)
                if line_de.get('branch') in ['信用中国(南通)'] and\
                len(info_split) == 16 and len(info_split[4]) > 4 and\
                len(info_split[4]) < 50 and is_Chinese(info_split[4][:4]) and info_split[1].isdigit():
                    skip_flag_list[i] = True
                    company = info_split[4]
                    if '号' in info_split[3]:
                        event_num = info_split[3]
                    else:
                        event_num = ''
                    if '20' in info_split[13] and len(info_split[13]) <= 10:
                        e_time = info_split[13]
                    else:
                        e_time = '0001-01-01'
                    if '20' in info_split[14] and len(info_split[14]) <= 10:
                        end_time = info_split[14]
                    else:
                        end_time = '0001-01-01'
                    branch_name = info_split[15]
                    company_name_tmp.append(company)
                    punish_type_tmp.append('')
                    penalty_amount_tmp.append('0.0')
                    involved_amount_tmp.append('0.0')
                    ...
                    end_time_tmp.append(end_time)
                    branch_name_tmp.append(branch_name)
                if line_de.get('branch') in ['信用中国(酒泉)'] and\
                len(info_split) == 5 and len(info_split[0]) > 4 and\
                len(info_split[0]) < 50 and is_Chinese(info_split[0][:4]) and '名称' not in info_split[0]:
                    skip_flag_list[i] = True
                    company = info_split[0]
                    if '号' in info_split[3]:
                        event_num = info_split[3]
                    ...
                    etime_tmp.append(e_time)
                    branch_name_tmp.append(branch_name)
                if line_de.get('branch') in ['信用中国(眉山)'] and\
                len(info_split) == 6 and len(info_split[2]) > 4 and\
                len(info_split[2]) < 50 and is_Chinese(info_split[2][:4]) and '名称' not in info_split[2]:
                    skip_flag_list[i] = True
                    company = info_split[2]
                    ...
                    punish_type_tmp.append('')
                    penalty_amount_tmp.append('0.0')
                    involved_amount_tmp.append(info_split[5])
                    punish_detail_tmp.append(row[:1000])
                    event_num_tmp.append(event_num)
                    branch_name_tmp.append(branch_name)
                if line_de.get('branch') in ['信用中国(眉山)'] and\
                len(info_split) == 7 and len(info_split[3]) > 4 and\
                len(info_split[3]) < 50 and is_Chinese(info_split[3][:4]) and '名称' not in info_split[3]:
                    skip_flag_list[i] = True
                    company = info_split[3]
                    event_num = ''
                    branch_name = info_split[2]
                    company_name_tmp.append(company)
                    punish_type_tmp.append('')
                    penalty_amount_tmp.append('0.0')
                    involved_amount_tmp.append('0.0')
...
for i, item in enumerate(data):
    ...
    #     reviewed = '22'
    if eventType == '失信黑名单' and len(title) < 4 and item.get('branch') in ['信用中国(北京)', '信用中国(大连)', '信用中国(亳州)', '信用中国(蚌埠)', '信用中国(淮南)', '信用中国(芜湖)', '信用中国(铜陵)', '信用中国(池州)', '信用中国(莆田)', '信用中国(鹰潭)', '信用中国(抚州)', '信用中国(淄博)', '信用中国(菏泽)', '信用中国(三门峡)', '信用中国(驻马店)', '信用中国(长沙)', '信用中国(岳阳)', '信用中国(张家界)', '信用中国(邵阳)', '信用中国(衡阳)', '信用中国(广东)', '信用中国(韶关)', '信用中国(河源)', '信用中国(潮州)', '信用中国(佛山)', '信用中国(东莞)', '信用中国(中山)', '信用中国(百色)', '信用中国(攀枝花)', '信用中国(南昌)', '信用中国(沧州)', '信用中国(苏州)']:
        reviewed = '22'

    if item.get('branch') == '信用中国(北京)':
        reviewed = '22'
    ...
    if item['branch'] == '信用中国(江西)':
        if len(item['contents']) == 2 and ',' in item['contents'][1]:
            content_list = item['contents'][1].split(',')
            for content in content_list:
                if '执行法院:' in content:
                    branch = content.split('执行法院:')[-1]
                    if len(branch) <= 2:
                        branch = item['branch']
                if '案号:' in content:
                    eventNum1 = content.split('案号:')[-1]
    if item['branch'] == '信用中国(汕头)':
        if len(item['contents']) == 2 and ',' in item['contents'][1]:
            content_list = item['contents'][1].split(',')
            for content in content_list:
                if '列入单位来源:' in content:
                    branch = content.split('列入单位来源:')[-1]
                    if len(branch) <= 2:
                        branch = item['branch']
    if item['branch'] == '信用中国(河南)':
        if len(item['contents']) == 1 and '\n' in item['contents'][0]:
            content_list = item['contents'][0].split('\n')
            if '案号' in content_list:
                p1 = content_list.index('案号')
                if '号' in content_list[p1 + 1]:
                    eventNum1 = content_list[p1 + 1]
                else:
                    eventNum1 = ''
            if '执行法院' in content_list:
                p2 = content_list.index('执行法院')
                if '法院' in content_list[p2 + 1]:
                    branch = content_list[p2 + 1]
                else:
                    branch = '信用中国(河南)'
    ...
    if company_name_list[i] != [''] and company_name_list[i] != '' and company_name_list[i] != []:
        ...
        for j, company in enumerate(company_name_list[i]):
            ...
            if skip_flag_list[i]:
                ...
                if len(ctime_list) > i and len(ctime_list[i]) > j:
                    ctimeDate = ctime_list[i][j].replace('年', '-').replace('月', '-').replace('日', '')
...
# 记录gtime数据log
# if logging:
#     write_log(db,mysql_log_name,mongo_name,add_data_num,max_gtime,last_gtime)
