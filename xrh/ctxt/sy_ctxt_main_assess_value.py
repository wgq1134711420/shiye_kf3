...
select   
  ...
  enDdate,
    totsharEqui   
     from sq_fin_probalsheetnew WHERE totsharEqui is not null
     ) as B 
where    A.innerCode=B.innerCode1 
         and DATEDIFF( STR_TO_DATE(A.tradeDate,'%Y%m%d'),STR_TO_DATE(B.endDate,'%Y%m%d'))>=0
...
from sy_ctxt_main_pbb_daily where tradeDate <= '{}' and tradeDate > '{}'  '''.format(max_trade_date,totmktcap_max_trade_date)
try:
    # 历史所有分为点计算
    peFiveQuantile = None
    peTenQuantile = None
...
( 
    select * from `sy_ctxt_main_pbb_daily` ,(select @gid:=null,@rn:=0 ) vars WHERE tradeDate <= '{}' and tradeDate>= '{}' and secuCode = '{}' order by pe  
        ...
        five_pe_rn_info = max(cur.fetchall())
...
( 
    select * from `sy_ctxt_main_pbb_daily` ,(select @gid:=null,@rn:=0 ) vars WHERE tradeDate <= '{}' and tradeDate>= '{}' and secuCode = '{}' order by pb  
        ...
        five_pb_rn_info = max(cur.fetchall())
...
( 
    select * from `sy_ctxt_main_pbb_daily` ,(select @gid:=null,@rn:=0 ) vars WHERE tradeDate <= '{}' and tradeDate>= '{}' and secuCode = '{}' order by pbb  
        ...
        five_pbb_rn_info = max(cur.fetchall())
...
( 
    select * from `sy_ctxt_main_pbb_daily` ,(select @gid:=null,@rn:=0 ) vars WHERE tradeDate <= '{}' and tradeDate>= '{}' and secuCode = '{}' order by pe  
        ...
        ten_pe_rn_info = max(cur.fetchall())
...
( 
    select * from `sy_ctxt_main_pbb_daily` ,(select @gid:=null,@rn:=0 ) vars WHERE tradeDate <= '{}' and tradeDate>= '{}' and secuCode = '{}' order by pb  
        ...
        ten_pb_rn_info = max(cur.fetchall())
...
( 
    select * from `sy_ctxt_main_pbb_daily` ,(select @gid:=null,@rn:=0 ) vars WHERE tradeDate <= '{}' and tradeDate>= '{}' and secuCode = '{}' order by pbb  
        ...
        ten_pbb_rn_info = max(cur.fetchall())
    ...
    print(e)
    print('ERORR插入表数据失败：',secuCode,tradeDate)
