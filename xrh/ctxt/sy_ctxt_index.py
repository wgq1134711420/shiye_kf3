...
# 获取数据源最大交易日期
max_trade_date_sql = 'select max(TradingDay) from index_derivpercentile'
cur2.execute(max_trade_date_sql)
max_trade_date_info = cur2.fetchone()
max_trade_date = max_trade_date_info[0]

# 获取目标表最大交易日期
index_max_trade_date_sql = 'select max(tradingDay) from sy_ctxt_index '
cur.execute(index_max_trade_date_sql)
index_max_trade_date_info = cur.fetchone()
index_max_trade_date = index_max_trade_date_info[0]
...
select_sql = '''SELECT C.ChiName,C.SecuCode,C.SecuAbbr,D.TradingDay,D.IndicatorType,D.StatisPeriod,D.IndiPercentile,'{}','{}','{}' from 
    ...
    LEFT JOIN secumain as B ON A.TargetIndexInnerCode=B.InnerCode) as C INNER JOIN 
        (SELECT * from index_derivpercentile WHERE IndicatorType in (1,2) and StatisPeriod in (60,120,999)
        and TradingDay <='{}' and TradingDay > '{}') as D
        ON C.TargetIndexInnerCode=D.IndexCode GROUP BY SecuCode,TradingDay,IndicatorType,StatisPeriod  ORDER BY SecuCode,TradingDay '''.format(
        isValid, createTime, dataState,max_trade_date,index_max_trade_date)

cur2.execute(select_sql)
add_infos = cur2.fetchall()
add_data_num = len(add_infos)

print(max_trade_date,index_max_trade_date,add_data_num)
...
if 850 > add_data_num or add_data_num > 980:
    print('ERORR数据入库数量异常')
