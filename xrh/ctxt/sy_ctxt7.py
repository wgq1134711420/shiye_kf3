...
inner_code_sql4 = '''SELECT innerCode from sy_ctxt_main_assess_value4 GROUP BY innerCode'''
cur.execute(inner_code_sql4)
inner_code_infos4 = cur.fetchall()
for inner_code_info4 in inner_code_infos4:
    inner_code_list.append(inner_code_info4[0])

# 未计算的innerCode
# all_trade_date_list2 = []
# for innerCode in all_trade_date_list:
#     innerCode += '\n'
#     all_trade_date_list2.append(innerCode)
# with open('innerCode.txt','w', encoding='utf-8') as f:
#     f.writelines(all_trade_date_list2)


all_inner_code_list = []
with open('/home/seeyii/xrh/ctxt/innerCode.txt', 'r', encoding="utf-8") as f:
    reader = f.readlines()
    for i in reader[1610:]:
...
for innerCode in all_inner_code_list:
   ...
   ( 
        select * from `sy_ctxt_main_pbb` ,(select @gid:=null,@rn:=0 ) vars WHERE tradeDate <= '{}' and tradeDate>= '{}' and secuCode = '{}' order by pbb  
            ...
            cur2.execute(ten_count_sql)
            ten_count_info = cur2.fetchone()
   ...
   ( 
        select * from `sy_ctxt_main_pbb` ,(select @gid:=null,@rn:=0 ) vars WHERE tradeDate <= '{}' and tradeDate>= '{}' and secuCode = '{}' order by pe  
            ...
            cur2.execute(ten_pe_rn_sql)
            ten_pe_rn_info = cur2.fetchone()
            if ten_pe_rn_info:
   ...
   ( 
        select * from `sy_ctxt_main_pbb` ,(select @gid:=null,@rn:=0 ) vars WHERE tradeDate <= '{}' and tradeDate>= '{}' and secuCode = '{}' order by pb  
            ...
            cur2.execute(ten_pb_rn_sql)
            ten_pb_rn_info = cur2.fetchone()
            if ten_pb_rn_info:
   ...
   ( 
        select * from `sy_ctxt_main_pbb` ,(select @gid:=null,@rn:=0 ) vars WHERE tradeDate <= '{}' and tradeDate>= '{}' and secuCode = '{}' order by pbb  
            ...
            cur2.execute(ten_pbb_rn_sql)
            ten_pbb_rn_info = cur2.fetchone()
            if ten_pbb_rn_info:
                ...
                pbbTenQuantile = None
            insert_sql = '''insert into sy_ctxt_main_assess_value4(chiName,innerCode,secuCode,tradeDate,totmktcap,
        ...
        print('插入表sy_ctxt_main_assess_value4数据成功')
