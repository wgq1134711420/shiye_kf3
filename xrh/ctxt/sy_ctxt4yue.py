...
# (
# SELECT * FROM sy_ctxt_main_totmktcap WHERE tradeDate <= '{}' and tradeDate > '{}'
# ) 
# as A
# ,
# (
...
#         max_trade_date,totmktcap_max_trade_date,max_trade_date,totmktcap_max_trade_date)
...
# select A.innerCode,A.tradeDate,B.*,DATEDIFF( STR_TO_DATE(A.tradeDate,'%Y%m%d'),STR_TO_DATE(B.endDate,'%Y%m%d')) as daynum from 
# (
# SELECT * FROM sy_ctxt_main_pe WHERE tradeDate <= '{}' and tradeDate > '{}'
# ) 
# as A
# ,
# (
...
# DATEDIFF( STR_TO_DATE(A.tradeDate,'%Y%m%d'),STR_TO_DATE(B.endDate,'%Y%m%d')) end) as daynum from 
# (
# SELECT * FROM sy_ctxt_main_pb WHERE tradeDate <= '{}' and tradeDate > '{}'
# ) 
# as A
# ,
# (
...
# print(all_secu_code_list[3200:3201],all_secu_code_list[3399:3400],len(all_secu_code_list))

# if len(all_secu_code_list) == 0:
#     print('There is no new data!')
#     os._exit(0)

# # 分为点计算
# for secuCode in all_secu_code_list[3200:3400]:
...
#             else:
#                 pbbTenQuantile = None
#             insert_sql = '''insert into sy_ctxt_main_assess_value_4yue(chiName,innerCode,secuCode,tradeDate,totmktcap,
...
#     print('插入表sy_ctxt_main_assess_value_4yue数据成功',secuCode)


# 导表(sy_ctxt_main_assess_value_4yue,1,2,3 to sy_ctxt_main_assess_value_daily) 129操作
Fouryue_sql = '''select chiName,innerCode,secuCode,tradeDate,totmktcap,
                        pe,peFiveQuantile,peTenQuantile,peQuantile,pb,pbFiveQuantile,pbTenQuantile,pbQuantile,
                        pbb,pbbFiveQuantile,pbbTenQuantile,pbbQuantile,isValid,createTime,dataState from sy_ctxt_main_assess_value_4yue_3'''
cur.execute(Fouryue_sql)
add_infos = cur.fetchall()
add_data_num = len(add_infos)
print('sy_ctxt_main_assess_value_4yue_3',add_data_num)
daily_sql = '''insert into sy_ctxt_main_assess_value_daily(chiName,innerCode,secuCode,tradeDate,totmktcap,
                        pe,peFiveQuantile,peTenQuantile,peQuantile,pb,pbFiveQuantile,pbTenQuantile,pbQuantile,
                        pbb,pbbFiveQuantile,pbbTenQuantile,pbbQuantile,isValid,createTime,dataState)
                        value (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)'''

cur.executemany(daily_sql, add_infos)
mysql_conn.commit()
print('sy_ctxt_main_assess_value_daily 插入数据成功')
# # log
# if logging == True:
