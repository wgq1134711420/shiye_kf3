...
cur2.execute(find_trade_date_sql)
find_trade_date_infos = cur2.fetchall()
...
for innerCode in ten_trade_date_list:
    i += 1
    select_sql = '''SELECT chiName,innerCode,secuCode,tradeDate,totmktcap,pe,pb,pbb from sy_ctxt_main_pbb where innerCode = '{}' '''.format(
        innerCode)
    try:
        # 历史所有分为点计算
        peFiveQuantile = None
   ...
   ( 
        select * from `sy_ctxt_main_pbb` ,(select @gid:=null,@rn:=0 ) vars WHERE tradeDate <= '{}' and secuCode = '{}' order by pe  
            ...
            cur2.execute(pe_rn_sql)
            pe_rn_info = cur2.fetchone()
            if pe_rn_info:
                pe_rn = pe_rn_info[0]
                if count > 1:
   ...
   ( 
        select * from `sy_ctxt_main_pbb` ,(select @gid:=null,@rn:=0 ) vars WHERE tradeDate <= '{}' and secuCode = '{}' order by pb  
            ...
            cur2.execute(pb_rn_sql)
            pb_rn_info = cur2.fetchone()
            if pb_rn_info:
                pb_rn = pb_rn_info[0]
                if count > 1:
   ...
   ( 
        select * from `sy_ctxt_main_pbb` ,(select @gid:=null,@rn:=0 ) vars WHERE tradeDate <= '{}' and secuCode = '{}' order by pbb  
            ...
            cur2.execute(pbb_rn_sql)
            pbb_rn_info = cur2.fetchone()
            if pbb_rn_info:
            ...
            WHERE tradeDate >= '{}' and tradeDate <= '{}' and secuCode = '{}' '''.format(five_trade_date, tradeDate,
                                                                                         secuCode)
            cur2.execute(five_count_sql)
            five_count_info = cur2.fetchone()
   ...
   ( 
    ...
    ) as A) as c on b.secuCode = c.secuCode and b.pe = c.pe '''.format(tradeDate, secuCode, tradeDate, five_trade_date,
                                                                       secuCode)
            cur2.execute(five_pe_rn_sql)
            five_pe_rn_info = cur2.fetchone()
            if five_pe_rn_info:
   ...
   ( 
    ...
    ) as A) as c on b.secuCode = c.secuCode and b.pb = c.pb '''.format(tradeDate, secuCode, tradeDate, five_trade_date,
                                                                       secuCode)
            cur2.execute(five_pb_rn_sql)
            five_pb_rn_info = cur2.fetchone()
            if five_pb_rn_info:
   ...
   ( 
        select * from `sy_ctxt_main_pbb` ,(select @gid:=null,@rn:=0 ) vars WHERE tradeDate <= '{}' and tradeDate>= '{}' and secuCode = '{}' order by pbb  
    ...
    ) as A) as c on b.secuCode = c.secuCode and b.pbb = c.pbb '''.format(tradeDate, secuCode, tradeDate,
                                                                         five_trade_date, secuCode)
            cur2.execute(five_pbb_rn_sql)
            five_pbb_rn_info = cur2.fetchone()
            if five_pbb_rn_info:
                ...
                pbbFiveQuantile = None
            insert_sql = '''insert into sy_ctxt_main_assess_value_ten2(chiName,innerCode,secuCode,tradeDate,totmktcap,
            ...
            cur.execute(insert_sql, (chiName, innerCode, secuCode, tradeDate, totmktcap,
                                     pe, peFiveQuantile, peTenQuantile, peQuantile, pb, pbFiveQuantile, pbTenQuantile,
                                     pbQuantile,
                                     pbb, pbbFiveQuantile, pbbTenQuantile, pbbQuantile, isValid, createTime, dataState))
            mysql_conn.commit()
        ...
        print(e)
    print('插入表sy_ctxt_main_assess_value_ten2数据成功')
    print(i, innerCode)
